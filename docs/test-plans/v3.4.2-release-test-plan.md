...
node tests/unit/test-table-sort.js          # NEW — shared table sort utility
node tests/unit/test-channel-colors.js      # NEW — channel color model
node tests/unit/test-panel-corner.js        # NEW — panel corner toggle
node tests/unit/test-packets.js             # NEW — packets page logic
node tests/unit/test-hop-resolver-affinity.js
node tests/unit/test-customizer-v2.js
node tests/unit/test-live.js
node tests/e2e/test-live-dedup.js
```

### E2E / Playwright
```bash
BASE_URL=http://localhost:13581 node tests/e2e/test-e2e-playwright.js
```
...
| No automated test for channel color persistence | Medium | `tests/unit/test-channel-colors.js` covers model; need Playwright for UI |
