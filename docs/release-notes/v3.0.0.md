...
At the time of this release, deployments could temporarily roll back to the previous Node.js backend. That backend and its dedicated Dockerfile were removed after the Go migration was established and are not supported by the current source tree.

For current deployments, roll back by redeploying a previously known-good Go image or release. Check out the v3.0.0 release tag only when reproducing the historical migration procedure; do not restore the removed Node.js backend to a current checkout.

See [the Go migration guide](../go-migration.md) for the full migration guide.
...
*Full migration guide: [Go migration guide](../go-migration.md)*
*Previous release: [v2.6.0](v2.6.0.md)*
