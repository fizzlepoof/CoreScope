...
async function main() {
  ...
  try {
    ...
    if (requireChromium) {
      console.error(`tests/e2e/test-gestures-1185-scroll-discriminator-e2e.js: FAIL — Chromium required but unavailable: ${err.message}`);
      process.exit(1);
    }
    console.log(`tests/e2e/test-gestures-1185-scroll-discriminator-e2e.js: SKIP — Chromium unavailable: ${err.message}`);
    process.exit(0);
  }
  ...
  await browser.close();
  console.log(`\ntests/e2e/test-gestures-1185-scroll-discriminator-e2e.js: ${passes} passed, ${failures} failed`);
  process.exit(failures > 0 ? 1 : 0);
}

main().catch((err) => { console.error('tests/e2e/test-gestures-1185-scroll-discriminator-e2e.js: FAIL —', err); process.exit(1); });
