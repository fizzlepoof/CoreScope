...
async function main() {
  ...
  try {
    ...
    if (process.env.CHROMIUM_REQUIRE === '1') {
      console.error(`tests/e2e/test-nav-priority-1391-e2e.js: FAIL — Chromium required but unavailable: ${err.message}`);
      process.exit(1);
    }
    console.log(`tests/e2e/test-nav-priority-1391-e2e.js: SKIP (Chromium unavailable: ${err.message.split('\n')[0]})`);
    process.exit(0);
  }
  ...
  console.log(`\ntests/e2e/test-nav-priority-1391-e2e.js: ${failures === 0 ? 'OK' : 'FAIL'} — ${passes}/${total} passed`);
  process.exit(failures === 0 ? 0 : 1);
}

main().catch((err) => {
  console.error('tests/e2e/test-nav-priority-1391-e2e.js: fatal', err);
  process.exit(1);
});
