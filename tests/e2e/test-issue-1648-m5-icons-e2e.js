...
async function main() {
  ...
  try {
    ...
    if (requireChromium) {
      console.error(`tests/e2e/test-issue-1648-m5-icons-e2e.js: HARD FAIL — Chromium unavailable: ${err.message}`);
      process.exit(1);
    }
  ...
  await browser.close();
  console.log(`\ntests/e2e/test-issue-1648-m5-icons-e2e.js: ${passes} passed, ${failures} failed`);
  assert.strictEqual(failures, 0, `${failures} M5 icon-render assertions failed`);
  process.exit(0);
}

main().catch((err) => {
  console.error('tests/e2e/test-issue-1648-m5-icons-e2e.js: FAIL —', err);
  process.exit(1);
});
